...
export default async function PanelCostsPage({ searchParams }: { searchParams?: Promise<{ month?: string; error?: string; success?: string }> }) {
  ...
  return (
    <>
      <PanelHeader title="Custos mensais" subtitle="Concentre aqui os custos fixos do período para o dashboard ficar limpo." monthRef={monthRef} />
      ...
      <section className="panel-card">
        <form action={savePanelCostsAction} className="panel-form two-col">
          ...
          <label className="full"><span>Mês de referência</span><input type="month" value={monthRef} readOnly /></label>
          ...
          <div className="full"><button className="panel-button" type="submit">Salvar custos</button></div>
        </form>
      </section>
    </>
  );
